...
def _add_task_obligated_agents(
    ...
    *,
    ...
    task context injection can force a directed reply.

    Args:
        active_agents: Current list of (agent_id, agent) tuples for this round.
        task_store: SimulationTaskStore for the running simulation.
        env: OASIS environment (provides env.agent_graph).
    ...
    try:
        ...
        already_active_ids = {aid for aid, _ in active_agents}
        name_to_id = {name: aid for aid, name in agent_names.items()}

        additions = []
        for name in obligated_names:
            agent_id = name_to_id.get(name)
            if agent_id is None or agent_id in already_active_ids:
                continue
            try:
                agent = env.agent_graph.get_agent(agent_id)
                additions.append((agent_id, agent))
            except Exception:
                pass
...
def _build_task_notification(task, completing_agent: str, parsed) -> str:
    """Build a notification string for the agent who originally assigned a task.

    Called after apply_task_action succeeds for 'complete' or 'update_status'
    actions so the assigner learns what happened and can react next round.

    Args:
        task: SimulationTask instance (already reflects the new status).
        completing_agent: Display name of the agent who acted on the task.
        parsed: ParsedTaskAction with the raw reason/output fields.

    Returns:
        A formatted notification string ready for memory injection.
    """
    display_id = task.issue_key or task.task_id[:8]

    if parsed.action_type == "complete":
        lines = [
            f'[Task Completed] {completing_agent} has completed your task "{task.title}" [{display_id}].',
        ]
        output = parsed.output or task.output
        if output:
            lines.append(f"Result: {output}")
        lines.append(
            f"Reply to {completing_agent} to acknowledge this result, give feedback, "
            f"or assign a follow-up task if needed."
        )
    elif task.status == "blocked":
        lines = [
            f'[Task Blocked] {completing_agent} is blocked on your task "{task.title}" [{display_id}].',
        ]
        reason = parsed.reason or parsed.output
        if reason:
            lines.append(f"Reason: {reason}")
        lines.append(
            f"REQUIRED: Reply to {completing_agent} to help unblock this task. "
            f"You should either clarify requirements or reassign part of the work."
        )
    else:
        lines = [
            f'[Task Update] {completing_agent} has updated your task "{task.title}" [{display_id}] '
            f"to status: {task.status}.",
        ]
        reason = parsed.reason or parsed.output
        if reason:
            lines.append(f"Note: {reason}")
        lines.append(
            f"You may reply to {completing_agent} to acknowledge or provide guidance."
        )
...
async def run_twitter_simulation(
    ...
    log_info(
        f"Simulation loop completed! Time elapsed: {elapsed:.1f}s, total actions: {total_actions}"
    )
...
async def main():
    ...
    log_manager.info(
        f"Command-waiting mode: {'enabled' if wait_for_commands else 'disabled'}"
    )
    log_manager.info("=" * 60)
    ...
    log_manager.info("=" * 60)
    log_manager.info(
        f"Simulation loop completed! Total time elapsed: {total_elapsed:.1f}s"
    )
