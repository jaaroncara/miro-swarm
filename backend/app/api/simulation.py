...
from ..tools.simulation_support import (
    check_simulation_prepared as tool_check_simulation_prepared,
)
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        if not entity:
            return (
                jsonify(
                    {"success": False, "error": f"Entity not found: {entity_uuid}"}
                ),
                404,
            )
...
def create_simulation():
    ...
    try:
        ...
        if not project_id:
            return (
                jsonify({"success": False, "error": "Please provide project_id"}),
                400,
            )
        ...
        session = WorkbenchSession.open(
            ...
            graph_id=graph_id,
            metadata={"entrypoint": "api.simulation.create"},
        )
...
def prepare_simulation():
    ...
    try:
        ...
        if not simulation_id:
            return (
                jsonify({"success": False, "error": "Please provide simulation_id"}),
                400,
            )

        session = WorkbenchSession.open(
            simulation_id=simulation_id,
            metadata={"entrypoint": "api.simulation.prepare"},
        )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            ...
            return (
                jsonify({"success": False, "error": f"Task not found: {task_id}"}),
                404,
            )
...
def delete_simulation(simulation_id: str):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Simulation deleted: {simulation_id}"}
        )
    except Exception as e:
...
def _parse_optional_bool_query(value: str | None):
...
def _task_error_response(exc: Exception):
    if isinstance(exc, TaskAuthorizationError):
        return jsonify({"success": False, "error": str(exc)}), 403
    if isinstance(exc, TaskLifecycleError):
        return jsonify({"success": False, "error": str(exc)}), 400
    raise exc


def _get_task_actor(data: dict):
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates through the reports directory to find reports matching the
    simulation_id. If there are multiple matches, returns the most recent one
...
@simulation_bp.route("/<simulation_id>/tasks", methods=["GET"])
def get_simulation_tasks(simulation_id: str):
    """Get simulation narrative tasks, with optional task filters."""
    try:
        assigned_to = request.args.get("assigned_to") or None
        assigned_by = request.args.get("assigned_by") or None
        issue_key = request.args.get("issue_key") or None
        status = request.args.get("status") or None
        if status is not None and status not in _TASK_API_VALID_STATUSES:
            valid_values = ", ".join(_TASK_API_VALID_STATUSES)
            return (
                jsonify(
                    {
                        "success": False,
                        "error": f"Invalid status '{status}'. Valid values: {valid_values}.",
                    }
                ),
                400,
            )

        completed = _parse_optional_bool_query(request.args.get("completed"))
        ...
        logger.error(f"Failed to get simulation tasks: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def create_simulation_task(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to create simulation task: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500


@simulation_bp.route("/<simulation_id>/tasks/<task_ref>", methods=["GET"])
def get_simulation_task(simulation_id: str, task_ref: str):
    """Get details for a single simulation task."""
    try:
...
def start_simulation_task(simulation_id: str, task_ref: str):
    ...
    try:
        ...
        logger.error(f"Failed to start simulation task: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500
        ...
        if file_exists:
            ...
            try:
                if platform == "reddit":
                    ...
                    profiles = SimulationManager._load_twitter_profiles_csv(
                        profiles_file
                    )
